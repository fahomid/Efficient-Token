...
/** Read `<projectRoot>/.claude/.efficient-token.alive`; never throws. */
export function readStatus(projectRoot: string): ServerStatus {
  const beat = path.join(projectRoot, ".claude", ".efficient-token.alive");
  try {
    const st = fs.statSync(beat);
    const ageMs = Date.now() - st.mtimeMs;
    if (ageMs > STALE_MS) return { up: false, ageMs };
    let data: Record<string, unknown> = {};
    try {
      const raw: unknown = JSON.parse(fs.readFileSync(beat, "utf8"));
    ...
    const num = (k: string): number | undefined => (typeof data[k] === "number" ? (data[k] as number) : undefined);
    return {
      up: true,
      ageMs,
      version: str("v"),
      ...
      calls: num("calls"),
      returnedTokens: num("returnedTokens"),
...
export function formatStatus(s: ServerStatus): string {
  ...
  return `efficient-token${ver}${tier}: up${usage}`;
}
