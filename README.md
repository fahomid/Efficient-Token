...
| `color_contrast` | Deterministic **color math**: WCAG **contrast ratio + AA/AAA** pass/fail between two colors, or **convert** one color between hex/rgb/hsl. Accepts hex (3/4/6/8), rgb()/hsl(), and common names. *(read-only)* |
...
| `call_hierarchy` | A function's local call neighborhood in one call: its **callees** (functions it calls, each with where it's defined) + its **callers** (workspace call sites). For callers only, `call_sites` is lighter. *(read-only)* |
| `marker_inventory` | Inventory **code-comment markers** (TODO/FIXME/HACK/XXX/BUG, or custom `tags`) across the workspace — grouped by tag, each `file:line + text` — matching only after a comment leader so prose isn't a false positive. *(read-only)* |
| `import_map` | A file's **dependency edges**: what it **imports** (workspace files vs external packages) and/or who **imports it** (dependents), resolved across the workspace, instead of grepping import lines. JS/TS family. *(read-only)* |
...
| `apply_patch` | Apply **many edits across one or more files in one atomic call** (all-or-nothing). Each edit is `{ file_path, old_string, new_string, replace_all? }` (like `code_edit`/`Edit`); validated (incl. syntax guard) in memory first — if anything fails, nothing is written. Cuts the round-trips of editing files one at a time. *(mutating)* |
| `move_symbol` | **Relocate a definition from one file to another** atomically, rewriting the **named imports/re-exports** of it across the workspace (and importing it back into the source if still used). `dryRun`-able, syntax-guarded; default/namespace importers + the moved code's own deps are flagged, not guessed. JS/TS. *(mutating)* |
